...
begin
  ...
  elsif new.agent_id = 'AG-ACTION'
    and new.agent_version = 'ag-action-1.0.0'
    and new.output_schema_version = 'capa_action_plan_advisory-1.0.0'
    and new.status = 'completed_draft'
  then
    if not exists (
      select 1 from public.capa_ai_reference_manifests as manifest
      where manifest.organization_id = new.organization_id
        ...
        and manifest.correlation_id = new.correlation_id
        and manifest.output_status = new.status
        and manifest.manifest_schema_version = 'capa-action-plan-advisory-reference-manifest-1.0.0'
    ) then
      raise exception using errcode = '23514', message = 'S60 AG-ACTION AI output requires an exact durable reference manifest.';
    end if;
  elsif new.agent_id = 'AG-REVIEW'
    and new.agent_version = 'ag-review-1.0.0'
    and new.output_schema_version = 'capa_action_plan_review_advisory-1.0.0'
    and new.status = 'completed_draft'
  then
    if not exists (
      select 1 from public.capa_ai_reference_manifests as manifest
      where manifest.organization_id = new.organization_id
        ...
        and manifest.correlation_id = new.correlation_id
        and manifest.output_status = new.status
        and manifest.manifest_schema_version = 'capa-action-plan-review-advisory-reference-manifest-1.0.0'
        and manifest.reference_manifest->>'source_case_version_id' = new.case_version_id::text
        and manifest.reference_manifest->>'action_plan_section_version_id' = new.output_payload->>'action_plan_section_version_id'
        and new.output_payload->>'source_case_version_id' = new.case_version_id::text
    ) then
      raise exception using errcode = '23514', message = 'S70 AG-REVIEW AI output requires an exact authoritative action-plan review reference manifest.';
    end if;
  end if;
  return new;
end;
...
comment on table public.capa_ai_reference_manifests is
  'Immutable server-only S40, S50, S60, and S70 reference provenance. Reference source IDs are never model prompt or AI output payload data.';
