...
} from "../application/capa-implementation-workspace-service";
import type {
  CapaApiLogger,
...
function errorResponse(
  ...
  return response({
    error: {
      code,
      message,
      correlation_id: traceValue.correlation_id,
    },
  }, status);
}

function projection(
  workspace: CapaImplementationWorkspaceProjection,
): CapaImplementationWorkspaceProjection {
  return {
...
function serviceError(
  traceValue: RequestTrace,
  result: CapaImplementationWorkspaceServiceResult,
): Response {
  if (result.status === "loaded" || result.status === "saved") {
    throw new Error("A successful implementation workspace result cannot be mapped as an error.");
  }
  switch (result.status) {
    case "not_found":
      return errorResponse(
        traceValue,
        404,
        "CAPA_IMPLEMENTATION_WORKSPACE_CASE_NOT_FOUND",
    ...
    case "authorization_denied":
      return errorResponse(
        traceValue,
        403,
        "CAPA_IMPLEMENTATION_WORKSPACE_ACCESS_DENIED",
        "The S80 implementation workspace operation is not authorized.",
      );
    case "workflow_conflict":
      return errorResponse(
        traceValue,
        409,
        "CAPA_IMPLEMENTATION_WORKSPACE_CASE_STATE_CONFLICT",
        "The CAPA case is not in S80 Implementation.",
      );
    case "baseline_unavailable":
      return errorResponse(
        traceValue,
        409,
        "CAPA_IMPLEMENTATION_BASELINE_NOT_AVAILABLE",
        "The approved S70 action-plan baseline is not available for this case.",
      );
    case "case_changed":
      return errorResponse(
        traceValue,
        ...
        "The CAPA case changed before this workspace operation could be completed.",
      );
    case "validation_failed":
      return errorResponse(
        traceValue,
        400,
        "INVALID_CAPA_IMPLEMENTATION_WORKSPACE_REQUEST",
        "The S80 implementation workspace request is invalid.",
      );
    case "concurrency_conflict":
      return errorResponse(
        traceValue,
        409,
        "WORKSPACE_DRAFT_CONCURRENCY_CONFLICT",
        "The S80 implementation workspace changed before this save could be completed.",
      );
    case "baseline_conflict":
      return errorResponse(
        traceValue,
        409,
        "CAPA_IMPLEMENTATION_BASELINE_CONFLICT",
        "The approved S70 action-plan baseline could not be preserved.",
      );
    case "persistence_failed":
      return errorResponse(
        ...
        500,
        "CAPA_INTERNAL_ERROR",
        "The S80 implementation workspace request could not be completed.",
      );
  }
}

async function sessionContext(
  dependencies: CapaImplementationWorkspaceApiDependencies,
  traceValue: RequestTrace,
...
export async function handleCapaImplementationWorkspaceGet(
  request: Request,
  caseId: string,
  dependencies: CapaImplementationWorkspaceApiDependencies,
): Promise<Response> {
  ...
  try {
    ...
    const result = await dependencies.create_workspace_service(context).load({
      capa_case_id: normalizedCaseId as CapaCaseId,
    });
    if (result.status === "loaded") {
      return response({
        workspace: projection(result.workspace),
    ...
    dependencies.logger.error(
      "CAPA API implementation workspace load failed.",
      {
        correlation_id: requestTrace.correlation_id,
    ...
    return errorResponse(
      ...
      500,
      "CAPA_INTERNAL_ERROR",
      "The CAPA implementation workspace request could not be completed.",
    );
  }
}

export async function handleCapaImplementationWorkspacePut(
  request: Request,
  caseId: string,
  dependencies: CapaImplementationWorkspaceApiDependencies,
): Promise<Response> {
  ...
  try {
    ...
    const result = await dependencies.create_workspace_service(context).save({
      capa_case_id: normalizedCaseId as CapaCaseId,
      body,
    ...
    dependencies.logger.error(
      "CAPA API implementation workspace save failed.",
      {
        correlation_id: requestTrace.correlation_id,
        error_name: error instanceof Error ? error.name : "UnknownError",
      },
