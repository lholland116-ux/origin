...
import type {
  ...
  CapaActionPlanWorkspaceDraftServiceResult,
} from "../application/capa-action-plan-workspace-draft-service";
...
export interface CapaActionPlanWorkspaceDraftApiDependencies {
  readonly get_session_facts: () => Promise<SupabaseCapaSessionFacts | null>;
  readonly resolve_context: SupabaseCapaContextResolver;
  readonly create_workspace_service: (context: CapaRequestContext) => CapaActionPlanWorkspaceDraftService;
  readonly now: () => Date;
...
function projection(workspace: NonNullable<Extract<CapaActionPlanWorkspaceDraftServiceResult, { readonly workspace: unknown }>["workspace"]>) {
  return {
...
function serviceError(traceValue: RequestTrace, result: CapaActionPlanWorkspaceDraftServiceResult): Response {
  if (result.status === "loaded" || result.status === "saved") {
    throw new Error("A successful action-plan workspace result cannot be mapped as an error.");
  }
  switch (result.status) {
    case "not_found":
      return errorResponse(traceValue, 404, "CAPA_ACTION_PLAN_WORKSPACE_CASE_NOT_FOUND", "The CAPA case was not found.");
    case "workflow_conflict":
      return errorResponse(traceValue, 409, "CAPA_ACTION_PLAN_WORKSPACE_CASE_STATE_CONFLICT", "The CAPA case is not in S60 Action Planning.");
    case "case_changed":
    ...
    case "authorization_denied":
      return errorResponse(traceValue, 403, "CAPA_ACTION_PLAN_WORKSPACE_ACCESS_DENIED", "The S60 action-planning workspace operation is not authorized.");
    case "validation_failed":
      return errorResponse(traceValue, 400, "INVALID_CAPA_ACTION_PLAN_WORKSPACE_REQUEST", "The S60 action-planning workspace request is invalid.");
    case "concurrency_conflict":
      return errorResponse(traceValue, 409, "WORKSPACE_DRAFT_CONCURRENCY_CONFLICT", "The action-planning workspace changed before this save could be completed.");
  }
}

async function sessionContext(
  request: Request,
  dependencies: CapaActionPlanWorkspaceDraftApiDependencies,
...
export async function handleCapaActionPlanWorkspaceDraftGet(
  request: Request,
  caseId: string,
  dependencies: CapaActionPlanWorkspaceDraftApiDependencies,
): Promise<Response> {
  ...
  try {
    ...
    dependencies.logger.error("CAPA API action-plan workspace load failed.", { correlation_id: requestTrace.correlation_id, error_name: error instanceof Error ? error.name : "UnknownError" });
    return errorResponse(requestTrace, 500, "CAPA_INTERNAL_ERROR", "The CAPA action-planning workspace request could not be completed.");
  }
}

export async function handleCapaActionPlanWorkspaceDraftPut(
  request: Request,
  caseId: string,
  dependencies: CapaActionPlanWorkspaceDraftApiDependencies,
): Promise<Response> {
  ...
  try {
    ...
    if (normalizedCaseId === null || normalizedCaseId !== caseId) return errorResponse(requestTrace, 400, "INVALID_CAPA_CASE_ID", "A valid CAPA case identifier is required.");
    let body: unknown;
    try { body = await request.json(); } catch { return errorResponse(requestTrace, 400, "INVALID_CAPA_ACTION_PLAN_WORKSPACE_REQUEST", "The S60 action-planning workspace request is invalid."); }
    ...
    if (error instanceof SupabaseCapaContextError) return errorResponse(requestTrace, 401, "INVALID_SESSION_CONTEXT", "The authenticated session is not valid for this request.");
    if (error instanceof SupabaseCapaTenantAccessError) return errorResponse(requestTrace, 403, "CAPA_TENANT_ACCESS_DENIED", "The authenticated user is not authorized to access a CAPA organization.");
    dependencies.logger.error("CAPA API action-plan workspace save failed.", { correlation_id: requestTrace.correlation_id, error_name: error instanceof Error ? error.name : "UnknownError" });
    return errorResponse(requestTrace, 500, "CAPA_INTERNAL_ERROR", "The CAPA action-planning workspace request could not be completed.");
  }
}
