...
import {
  ...
  type CapaInvestigationPlanningAdvisoryService,
} from "../ai/capa-investigation-planning-advisory-service";

import {
  CapaInvestigationPlanningAdvisoryValidationError,
  validateCapaInvestigationPlanningAdvisoryBrowserRequest,
} from "../ai/capa-investigation-planning-advisory-validator";
...
export interface CapaInvestigationPlanningAdvisoryApiDependencies {
...
function requestTrace(
  ...
  return {
    request_id: (normalizedUuid(request.headers.get("x-request-id")) ??
      generateUuid()) as RequestId,
    correlation_id: (normalizedUuid(request.headers.get("x-correlation-id")) ??
      generateUuid()) as CorrelationId,
  };
}
...
function serviceErrorResponse(
  trace: { readonly correlation_id: CorrelationId },
  error: CapaInvestigationPlanningAdvisoryServiceError,
): Response | null {
  switch (error.reason_code) {
    ...
    case "CASE_NOT_IN_INVESTIGATION_PLANNING":
      return errorResponse(
        trace,
        409,
        "CAPA_ADVISORY_CASE_STATE_CONFLICT",
        "The CAPA case is not in the required state for investigation-planning advisory.",
      );
    case "ADVISORY_ACCESS_DENIED":
      return errorResponse(
        ...
        "The CAPA investigation-planning advisory operation is not authorized.",
      );
    case "AGENT_NOT_ELIGIBLE":
      return errorResponse(
        ...
        "The governed investigation-planning advisory is not available for the current CAPA state.",
      );
...
export async function handleCapaInvestigationPlanningAdvisoryPost(
  request: Request,
  caseId: string,
  dependencies: CapaInvestigationPlanningAdvisoryApiDependencies,
): Promise<Response> {
  ...
  try {
    ...
    if (!body.valid) {
      return errorResponse(
        ...
        "The CAPA investigation-planning advisory request is invalid.",
      );
    }

    const advisoryRequest =
      validateCapaInvestigationPlanningAdvisoryBrowserRequest(body.value);
    ...
    if (error instanceof CapaInvestigationPlanningAdvisoryValidationError) {
      return errorResponse(
        trace,
        400,
        "INVALID_CAPA_ADVISORY_REQUEST",
    ...
    if (error instanceof CapaInvestigationPlanningAdvisoryServiceError) {
      const mapped = serviceErrorResponse(trace, error);
      if (mapped !== null) return mapped;
    }

    dependencies.logger.error("CAPA API investigation-planning advisory failed.", {
      correlation_id: trace.correlation_id,
