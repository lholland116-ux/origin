...
import { CapaActionPlanAdvisoryOutputValidationError } from "./capa-action-plan-advisory-validator";
...
export class CapaActionPlanAdvisoryServiceError extends Error { constructor(readonly reason_code: CapaActionPlanAdvisoryServiceReasonCode, readonly diagnostic_cause_name?: string) { super("The governed S60 action-plan advisory operation failed."); this.name = "CapaActionPlanAdvisoryServiceError"; } }
...
export class CapaActionPlanAdvisoryService {
  ...
  async execute(invocation: CapaActionPlanAdvisoryInvocation): Promise<{ readonly advisory: CapaActionPlanAdvisoryResponse; readonly snapshot: Readonly<{ capa_case_id: CapaCaseId; case_version_id: CapaCaseVersionId; record_version: number }> }> {
    ...
    let generated: Awaited<ReturnType<CapaActionPlanAdvisoryGenerator["generate"]>>; try { generated = await this.dependencies.generator.generate({ context: resolution.assembly, request_id: invocation.request_id, correlation_id: invocation.correlation_id }); } catch (error) { throw new CapaActionPlanAdvisoryServiceError("ADVISORY_GENERATION_FAILED", error instanceof Error ? error.name : "UnknownError"); }
    if (!validGenerated(generated, context, invocation) || !validateCapaActionPlanAdvisoryAuthoritativeBindings(generated.response, context, resolution.assembly.reference_manifest)) throw new CapaActionPlanAdvisoryServiceError("INVALID_ADVISORY_RESULT");
