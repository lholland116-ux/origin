import { validateCapaActionPlan, type CapaActionPlanContent } from "../domain/capa-action-plan";
import {
...
export type CapaActionPlanAdvisoryValidationLocation =
  | "root" | "proposal" | "concern" | "suggestion" | "effectiveness_suggestion" | "proposed_action_plan";
...
export function validateCapaActionPlanAdvisoryModelOutput(value: string): RawCapaActionPlanAdvisoryModelOutput {
  ...
  if (!object(proposal) || !exact(proposal, ["advisory_summary", "completeness_linkage_concerns", "action_improvements", "effectiveness_planning_improvements", "proposed_action_plan"]) || !text(proposal.advisory_summary)) fail("INVALID_SHAPE", "proposal");
