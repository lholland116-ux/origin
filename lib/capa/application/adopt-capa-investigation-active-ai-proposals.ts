...
type Success = { readonly status: "adopted" | "already_adopted"; readonly records: readonly PersistedCapaInvestigationActiveAdoption[] };
...
function stableSelected(intent: CapaInvestigationActiveAdoptionIntentRequest, resolved: Awaited<ReturnType<CapaInvestigationActiveAdoptionSourceResolver["resolve"]>> & { readonly status: "resolved" }, normalizedContent: ReadonlyMap<string, unknown>) {
  ...
  return [...intent.selected_items].sort((a, b) => Number(a.proposal_key.slice(1)) - Number(b.proposal_key.slice(1))).map((item) => ({ proposal_key: item.proposal_key, proposal_category: byKey.get(item.proposal_key as never)?.proposal_category, adopted_content: normalizedContent.get(item.proposal_key) }));
}
...
export async function adoptCapaInvestigationActiveAiProposals(dependencies: AdoptCapaInvestigationActiveAiProposalsDependencies, command: AdoptCapaInvestigationActiveAiProposalsCommand): Promise<AdoptCapaInvestigationActiveAiProposalsResult> {
  ...
  const inputs = intent.selected_items.map((item) => {
    ...
    const adoption = constructCapaInvestigationActiveAdoption({ adoption_id: dependencies.id_generator.generateAdoptionId(), organization_id: dependencies.tenant.organization_id, capa_case_id: command.capa_case_id, case_version_id: intent.expected_case_version_id, record_version: intent.expected_record_version, output_id: intent.output_id, proposal_key: item.proposal_key, proposal_category: proposal.proposal_category, adopted_item: { proposal_key: item.proposal_key, adopted_content: adoptedContent }, resolved_reference_bindings: proposal.resolved_reference_bindings, reference_manifest_schema_version: resolved.reference_manifest_schema_version, reference_manifest_fingerprint_algorithm: resolved.reference_manifest_fingerprint_algorithm, reference_manifest_sha256: resolved.reference_manifest_sha256, adopted_at: adoptedAt, adopted_by: dependencies.adopter, adoption_policy_version: CAPA_INVESTIGATION_ACTIVE_ADOPTION_POLICY_VERSION, request_id: command.request_trace.request_id, correlation_id: command.request_trace.correlation_id, idempotency_key: key as never, workflow_mutated: false, controlled_record_mutated: false, gate_approved: false });
