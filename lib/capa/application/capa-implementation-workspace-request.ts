...
export function validateCapaImplementationWorkspaceSaveRequest(
  ...
  const returnResponse = hasReturnResponse && value.implementation_review_return_response !== null
    ? validateCapaImplementationReviewReturnResponseEditableContent(
        value.implementation_review_return_response,
      )
    : null;
  if (returnResponse?.status === "invalid") {
    return {
      status: "invalid",
      reason_code: "INVALID_IMPLEMENTATION_WORKSPACE_REQUEST_DRAFT",
      detail_reason_code: returnResponse.reason_code,
    };
  }
