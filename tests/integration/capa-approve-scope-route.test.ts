...
vi.mock(
  "../../lib/capa/application/approve-capa-scope",
  () => ({
    approveCapaScope:
      mocks.approve,
  }),
...
const APPROVE_DEPENDENCIES =
  Object.freeze({
    marker:
      "approve-scope-dependencies",
  });
...
function validBody() {
  return {
    ...
    approval: {
      ...
      confirmation:
        "G01_SCOPE_ACCEPTANCE_CONFIRMED",

      rationale:
        "I reviewed the scope and confirm it is adequate for G-01.",
    },
  };
}
...
function request(
  ...
  return new Request(
    `http://localhost/api/capa/${CASE_ID}/approve-scope`,
    {
      method:
        "POST",
...
describe(
  "CAPA G-01 scope-approval handler",
  () => {
    it(
      "returns 401 without an authenticated session",
      async () => {
        const response =
          await handleCapaApproveScope(
            request(
              validBody(),
            ),
    ...
    it(
      "returns controlled G-01 blocker codes as response issues",
      async () => {
        mocks.approve.mockResolvedValue({
          status:
            "gate_blocked",
    ...
    it(
      ...
      async () => {
        ...
        expect(
          ...
          {
            message:
              "CAPA API scope approval failed.",
