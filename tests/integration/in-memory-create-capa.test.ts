...
function requestTrace(): RequestTrace {
  return {
    ...
    idempotency_key:
      "integration-create-capa-001" as IdempotencyKey,
  };
}
...
function tenantContext(): TenantContext {
  return {
    ...
    access_grant_id:
      "2bf86821-80f-42df-96f3-9e7ae645c061" as TenantAccessGrantId,
...
function command(): CreateCapaCommand {
  return {
...
function createDatabase(): InMemoryCapaDatabase {
...
describe("in-memory CAPA creation integration", () => {
  it(
    "commits the case, section, version and audit event atomically",
    async () => {
      ...
      const storedCase = await database.findCaseById(
        ORGANIZATION_ID,
        result.capa_case.capa_case_id,
      );

      const storedVersion =
        await database.findCaseVersionById(
          ORGANIZATION_ID,
          result.capa_case.capa_case_id,
          result.case_version.case_version_id,
        );

      const storedSection =
        await database.findSectionVersionById(
          ORGANIZATION_ID,
          result.capa_case.capa_case_id,
          result.section_version.section_version_id,
        );
      ...
      expect(auditPage.events).toHaveLength(1);
      expect(auditPage.events[0]).toEqual(
        storedAudit,
      );
      expect(auditPage.next_cursor).toBeUndefined();
    },
  );

  it(
    "rolls back all CAPA writes when the audit append fails",
    async () => {
      const database = createDatabase();
      ...
      const failingAuditRepository: AuditRepository = {
        ...
        async findEventById(
          ...
          return database.findEventById(
            organizationId,
            eventId,
          );
        },

        async listEventsForAggregate(query) {
          return database.listEventsForAggregate(
            query,
          );
        },
      };
  ...
  it(
    "does not expose committed records across organization boundaries",
    async () => {
      const database = createDatabase();
      ...
      const otherOrganizationAuditPage =
        await database.listEventsForAggregate({
          organization_id:
            OTHER_ORGANIZATION_ID,
          aggregate_type:
      ...
      expect(
        otherOrganizationAuditPage.events,
      ).toEqual([]);
    },
  );
  ...
  it(
    ...
    async () => {
      ...
      const firstRead = await database.findCaseById(
        ORGANIZATION_ID,
        CAPA_CASE_ID,
      );

      const secondRead = await database.findCaseById(
        ORGANIZATION_ID,
        CAPA_CASE_ID,
      );

      expect(firstRead).toEqual(result.capa_case);
      expect(secondRead).toEqual(result.capa_case);
