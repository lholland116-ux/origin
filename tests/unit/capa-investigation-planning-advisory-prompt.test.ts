...
describe("S30 investigation-planning advisory prompt", () => {
  ...
  it("requires the CS1 output shape, advisory flags, proposal keys, and question grammar", () => {
    ...
    expect(prompt).toContain("investigation_question");
    expect(prompt).toContain("due_date_consideration");
    expect(prompt).toContain("human_review_question");
    expect(prompt).toContain("verification_question");
    expect(prompt).toContain("one atomic single-clause question");
    expect(prompt).toContain("exactly one question mark");
    expect(prompt).toContain("Every due_date_consideration must be phrased as a human planning question");
    expect(prompt).toContain("must not state, assign, establish, or commit an authoritative due date");
    expect(prompt).toContain("Every investigation_question must follow this same controlled grammar");
    ...
    expect(prompt).toContain("Noun-phrase coordination with and may be used where needed");
    expect(prompt).toContain("and must not join two independent questions, predicates, or requested facts");
    expect(prompt).not.toContain("connector words: and,");
  });
});
