...
function serviceAuthentication():
  ...
  return {
    principal: {
      ...
      service_identity_id:
        "ea2cc413-0e7b-4488-a75e-0930c64472a7" as
          ServiceIdentityId,
    },

    session_id:
      "587f8fc1-ae91-4eb7-96b7-84a4339aaec6" as
        SessionId,
...
function tenantContext():
  ...
  return {
    ...
    access_grant_id:
      "2bf86821-80f7-42df-96f3-9e7ae645c061" as
        TenantAccessGrantId,
...
interface Harness {
  ...
  readonly write_order:
    string[];

  readonly inserted_cases:
    CapaCase[];

  readonly inserted_versions:
    CapaCaseVersion[];

  readonly inserted_sections:
    CapaSectionVersion[];

  readonly appended_events:
    AuditEvent[];

  readonly policy_requests:
    CapaPolicyEvaluationRequest[];

  readonly transaction_requests:
    RequestTrace[];
...
function createHarness():
  ...
  const transactionContext:
    TransactionContext = {
    transaction_id:
      "a33f15a0-14d1-42c8-93d7-bba2393c2959" as
        TransactionId,
...
describe(
  "createCapa authorization",
  () => {
    it(
      "denies an inactive session before policy evaluation",
      async () => {
        const harness =
          createHarness();
...
describe(
  "createCapa request validation",
  () => {
    it(
      "returns normalized validation issues without allocating or writing",
      async () => {
        const harness =
          createHarness();
...
describe(
  "createCapa atomic record creation",
  () => {
    it(
      "allocates and creates the aggregate, section, version and audit event in order",
      async () => {
        const harness =
          createHarness();
        ...
        expect(
          harness.inserted_cases,
        ).toHaveLength(1);

        expect(
          harness.inserted_sections,
        ).toHaveLength(1);

        expect(
          harness.inserted_versions,
        ).toHaveLength(1);

        expect(
          harness.appended_events,
        ).toHaveLength(1);

        const capaCase =
          harness.inserted_cases[0];

        const section =
          harness.inserted_sections[0];

        const version =
          harness.inserted_versions[0];

        const audit =
          harness.appended_events[0];
    ...
    it(
      "records service attribution when policy permits it",
      async () => {
        const harness =
          createHarness();
    ...
    it(
      "accepts an exact idempotent audit append",
      async () => {
        const harness =
          createHarness();
    ...
    it(
      "fails on an audit identity conflict",
      async () => {
        const harness =
          createHarness();
    ...
    it(
      "propagates a case-number allocation failure before business writes",
      async () => {
        const harness =
          createHarness();
    ...
    it(
      "propagates a business-write failure before audit append",
      async () => {
        const harness =
          createHarness();
    ...
    it(
      "sends the expected final-policy context",
      async () => {
        const harness =
          createHarness();
